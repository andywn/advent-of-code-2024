...
//            "p=9,5 v=-3,-3")
    ...
    lines.map { it -> Robot(it, 100, 102).process(100) }
        .forEach{ it -> when(it.quadrant(100, 102)) {
            (1) -> q1++
            (2) -> q2++
            (3) -> q3++
            (4) -> q4++
    ...
    for (i in 0..10403) {
        ...
        q3 = 0
        q4 = 0
        lines.map { it -> Robot(it, 100, 102).process(i) }
            .forEach{ it -> when(it.quadrant(100, 102)) {
                (1) -> q1++
                (2) -> q2++
                (3) -> q3++
        ...
        if (q1 * q2 * q3 * q4 < 60000000) {
            println("$i -> ${q1 * q2 * q3 * q4}")
        }
    }
}
...
class Robot(val line: String, val maxX: Int, val maxY: Int) {
    ...
    fun process(times: Int): Coords {
        ...
        return startingCoords.addCoord(velocity.multiply(times)).wrap(maxX, maxY)
    }
}